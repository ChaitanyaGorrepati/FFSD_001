...
avatarFile?.addEventListener("change", (e) => {
  ...
  reader.onload = () => {
    ...
    // Also sync sidebar/topbar avatars
...
function loadProfile() {
  ...
  if (deptEl && profile.department) deptEl.value = profile.department;
...
document.getElementById("save-profile-btn")?.addEventListener("click", () => {
  ...
  if (!updated.avatarSrc) syncAvatarEls(null, ((firstName[0] || "") + (lastName[0] || "")).toUpperCase() || "?");
...
confirmPwEl?.addEventListener("input", () => {
  ...
  if (!hint) return;
  hint.style.display = confirmPwEl.value && confirmPwEl.value !== newPwEl?.value ? "block" : "none";
});
  ...
  list.innerHTML = sessions.map(s => `
    <div class="session-row">
      ...
      <div class="session-info">
        <div class="session-device">${s.device} ${s.current ? '<span class="session-current-badge">Current</span>' : ""}</div>
...
function loadNotifPrefs() {
  const prefs = loadStored(NOTIF_KEY, {
    newCase: true, statusChange: true, transfer: true,
    comment: false, system: true, weekly: false
  });
  document.getElementById("notif-new-case").checked     = prefs.newCase     ?? true;
  document.getElementById("notif-status-change").checked = prefs.statusChange ?? true;
  document.getElementById("notif-transfer").checked     = prefs.transfer     ?? true;
  document.getElementById("notif-comment").checked      = prefs.comment      ?? false;
...
document.getElementById("save-notif-btn")?.addEventListener("click", () => {
  const prefs = {
    newCase:      document.getElementById("notif-new-case").checked,
